...
namespace Rex.Tools.Test.DataCheck.Entity
{
    ...
    public class NpgTableLayoutInfo
    {
        private string _tableName;
        private string _displayName;
        private List<NpgColumnInfo> _columns;

        internal NpgTableLayoutInfo(string tableName, DataTable layoutTable)
        {
            this._tableName = tableName;
            this._columns = new List<NpgColumnInfo>();
            foreach (DataRow row in layoutTable.Rows)
            {
                this._columns.Add(new NpgColumnInfo(row));
            }
        }

        public NpgTableLayoutInfo(string tableName)
        {
            this._tableName = tableName;
            this._columns = new List<NpgColumnInfo>();
        }
        ...
        public List<NpgColumnInfo> Columns
        {
            get
            {
                return _columns;
        ...
        public string GetDropDescriptionScript()
        {
            ...
            foreach (NpgColumnInfo col in this.Columns)
            {
                //列表示名削除
        ...
        private string CreateColumns()
        {
            StringBuilder sb = new StringBuilder();
            int index = 0;
            foreach (NpgColumnInfo column in this.Columns)
            {
                index++;
        ...
        private string CreateIndexSql()
        {
            ...
            if (keyCount > 0)
            {
                ...
                sb.AppendLine("(");
                int index = 0;
                foreach (NpgColumnInfo col in indexCols)
                {
                    index++;
    ...
    public partial class NpgColumnInfo
    {
        ...
        internal NpgColumnInfo(DataRow row)
        {
