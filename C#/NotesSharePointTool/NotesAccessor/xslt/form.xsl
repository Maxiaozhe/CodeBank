...
********************************************************************** -->
  ...
  <!-- XSL作成モードフラグ -->
  <xsl:param name="CreateMode">aspx</xsl:param>
  <!-- URLホットスポットフラグ -->
  ...
  <!--cssfile名-->
  <xsl:param name="cssfile">form1.css</xsl:param>
  <!--
**********************************************************************
変数定義
********************************************************************** -->
  <!-- Dominoバージョン -->
  <xsl:variable name="nd_version">
    <xsl:value-of select="dxl:document/@version"/>
  </xsl:variable>
  <!-- ユニバーサルID -->
  <xsl:variable name="unid">
    <xsl:value-of select="dxl:document/dxl:noteinfo/@unid"/>
  </xsl:variable>
  <!-- レプリカID -->
  <xsl:variable name="replicaid">
    <xsl:value-of select="dxl:document/@replicaid"/>
  </xsl:variable>
  <!-- 空文字 -->
  <xsl:variable name="kara">
    <xsl:text> </xsl:text>
  </xsl:variable>
  <!-- アンパサンド -->
  ...
  <!-- デフォルトフォントファミリー -->
  <xsl:variable name="SansSerifFamily">
    <xsl:value-of select="$SansSerif"/>
    <xsl:text> sans-serif</xsl:text>
  </xsl:variable>
  <xsl:variable name="SerifFamily">
    <xsl:value-of select="$Serif"/>
    <xsl:text> serif</xsl:text>
  </xsl:variable>
  <xsl:variable name="MonospaceFamily">
...
********************************************************************** -->
  <xsl:template name="outside-attachment-block">
    <xsl:element name="xsl:if">
      <xsl:attribute name="test">
        <xsl:text>$OutsideAttachments!=''</xsl:text>
      </xsl:attribute>
      <xsl:element name="xsl:call-template">
        <xsl:attribute name="name">
          <xsl:text>OutputOutsideAttachments</xsl:text>
        </xsl:attribute>
      </xsl:element>
    </xsl:element>
  </xsl:template>
  ...
  <xsl:template name="OutputOutsideAttachments">
    <xsl:element name="div">
      <xsl:attribute name="class">
        <xsl:text>defaultclass0</xsl:text>
      </xsl:attribute>
      <xsl:element name="hr"/>
      <xsl:element name="p">
      ...
      <xsl:element name="div">
        <xsl:attribute name="style">
          <xsl:text>cursor:pointer;margin-left:0px;padding-left:20px;background:url(></xsl:text>
          <xsl:value-of select="$imgsrc"/>
          <xsl:text>section_d.gif ) no-repeat;zoom:1;</xsl:text>
        </xsl:attribute>
        <xsl:attribute name="onclick">
          <xsl:text></xsl:text>section_expand('OutsideAttachments')
        </xsl:attribute>
        <xsl:element name="p">
          <xsl:attribute name="style">
            <xsl:text>text-indent:0in;text-align:left;</xsl:text>
          </xsl:attribute>
          <xsl:element name="font">
            <xsl:attribute name="style">
              <xsl:text>white-space:normal;font-size:10pt;</xsl:text>
            </xsl:attribute>
  ...
  <xsl:template name="OutputOutsideAttachment">
    ...
    <xsl:if test="$attachmentName!=''">
      ...
      <xsl:value-of select='$imgsrc'/>
      <xsl:value-of select="$AttachmentDirectory"/>
      <xsl:value-of select="substring-after($attachmentName, ':')"/>
      <xsl:text disable-output-escaping='yes'>"></xsl:text>
      <xsl:element name="span">
        <xsl:attribute name="style">
          <xsl:text>display:inline-block;text-indent:0in;background: url(</xsl:text>
          <xsl:value-of select="$imgsrc" />
          <xsl:text>attachicon.gif</xsl:text>
        ...
        <xsl:element name="font">
          ...
          <xsl:attribute name="style">
            ...
            <xsl:text>;font-size:9pt;font-family:</xsl:text>
...
********************************************************************** -->
  <xsl:template name="pardef">
    ...
    <xsl:variable name="leftmargin">
      <xsl:choose>
        <xsl:when test="count(@leftmargin)>0">
    ...
    <xsl:text>
		.</xsl:text>
    <xsl:if test="$CreateMode='xsl'">Form</xsl:if>
    <xsl:value-of select="$rtitemname"/>
            ...
            <xsl:attribute name="style">
              ...
              <xsl:choose>
                <xsl:when test="@borderstyle='shadow'">
                  ...
                  <xsl:text>;</xsl:text>
                  <xsl:text>border-left:solid 1px </xsl:text>
                  <xsl:value-of select="@color"/>
                  <xsl:text>;</xsl:text>
                  <xsl:text>border-right:solid 2px </xsl:text>
                  <xsl:value-of select="@color"/>
                  <xsl:text>;</xsl:text>
                  <xsl:text>border-bottom:solid 2px </xsl:text>
                  <xsl:value-of select="@color"/>
                  <xsl:text>;</xsl:text>
                ...
                <xsl:when test="@borderstyle='triple'">
                  <xsl:text>border-bottom:solid 3px </xsl:text>
                  <xsl:value-of select="@color"/>
                  <xsl:text>;</xsl:text>
                  <xsl:text>text-align:left;</xsl:text>
                </xsl:when>
                <xsl:when test="@borderstyle='twoline'">
                  <xsl:text>border-bottom:double 3px </xsl:text>
                  <xsl:value-of select="@color"/>
                  <xsl:text>;</xsl:text>
                  <xsl:text>text-align:left;</xsl:text>
                </xsl:when>
                ...
                <xsl:when test="@borderstyle='gradient'">
                  ...
                  <xsl:with-param name="dir" select="left" />
                  <xsl:with-param name="scolor" select="@color" />
                  <xsl:with-param name="ecolor" select="white" />
                  </xsl:call-template>
                  <xsl:text>text-align:left;</xsl:text>
              ...
              <xsl:when test="@borderstyle='tab' or @borderstyle='tabdiag'">
                <xsl:element name="span">
                  <xsl:attribute name="style">
                    ...
                    <xsl:value-of select="@color"/>
                    <xsl:text>;</xsl:text>
                    <xsl:if test="@borderstyle='tabdiag'">
                      <xsl:text>padding-left:28px;padding-right:28px;</xsl:text>
                    </xsl:if>
                    <xsl:if test="@borderstyle='tab'">
                      <xsl:text>margin-left:5px;padding-left:3px;padding-right:3px;</xsl:text>
                    </xsl:if>
                    <xsl:if test="count(child::dxl:font)=0">
                      <xsl:text>font-size:10pt;</xsl:text>
                    </xsl:if>
                  </xsl:attribute>
...
********************************************************************** -->
  <xsl:template match="dxl:par">
    <xsl:variable name="rtitemname" select="ancestor::dxl:field/@name"/>
      ...
      <xsl:attribute name="class">
        ...
        <xsl:choose>
          <xsl:when test="contains($def, $v_var)">
            <xsl:value-of select="substring-before($def, $v_var)"/>
            <xsl:text>class</xsl:text>
            <xsl:value-of select="substring-after($def, $v_var)"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="$rtitemname"/>
            <xsl:text>class</xsl:text>
            ...
            <xsl:if test="$liststyle!='' and $liststyle!='none'">
              <xsl:element name="img">
                <xsl:attribute name="src">
                  ...
                  <xsl:choose>
                    <xsl:when test="$liststyle='bullet'">
                      <xsl:text>bullet.gif</xsl:text>
                    </xsl:when>
                    <xsl:when test="$liststyle='check'">
                      <xsl:text>check.gif</xsl:text>
                    </xsl:when>
                    <xsl:when test="$liststyle='uncheck'">
                      <xsl:text>uncheck.gif</xsl:text>
                    </xsl:when>
                    <xsl:when test="$liststyle='circle'">
                      <xsl:text>circle.gif</xsl:text>
                    </xsl:when>
                    <xsl:when test="$liststyle='square'">
                      <xsl:text>square.gif</xsl:text>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:text>bullet.gif</xsl:text>
                    </xsl:otherwise>
    ...
    <xsl:element name="a">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:element name="img">
            <xsl:attribute name="src">
              <xsl:text>../../domino/dblink.gif</xsl:text>
            </xsl:attribute>
            <xsl:attribute name="style">
    ...
    <xsl:element name="a">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:element name="img">
            <xsl:attribute name="src">
              <xsl:text>../../domino/doclink.gif</xsl:text>
            </xsl:attribute>
            <xsl:attribute name="style">
              <xsl:text>border-width:0px;</xsl:text>
            </xsl:attribute>
    ...
    <xsl:element name="a">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:element name="img">
            <xsl:attribute name="src">
              <xsl:text>../../domino/viewlink.gif</xsl:text>
            </xsl:attribute>
            <xsl:attribute name="style">
              <xsl:text>border-width:0px;</xsl:text>
            </xsl:attribute>
  ...
  <xsl:template name="runelement">
    ...
    <xsl:choose>
      <xsl:when test="string-length($text)=0 and string-length($innertext)=0">
          <xsl:if test="count(descendant::dxl:field)>0">
          <xsl:element name="font">
            <xsl:attribute name="style">
              <xsl:choose>
            ...
            <xsl:choose>
              <xsl:when test="count(descendant::dxl:field)>0">
                <xsl:apply-templates />
              </xsl:when>
              <xsl:otherwise>
                <!--<xsl:value-of disable-output-escaping="yes" select="concat($amp, '#160;')"/>-->
      ...
      <xsl:otherwise>
        <xsl:element name="font">
          ...
          <xsl:attribute name="style">
            <xsl:if test="$highlight='blue'">
              <xsl:text>background-color:lightblue;</xsl:text>
            </xsl:if>
            <xsl:if test="$highlight='pink'">
              <xsl:text>background-color:lightpink;</xsl:text>
            </xsl:if>
            <xsl:if test="$highlight='yellow'">
          ...
          <xsl:if test="dxl:font">
            ...
            <xsl:if test="contains(dxl:font/@style,'superscript')">
              ...
              <xsl:text disable-output-escaping='yes'>&lt;Sup style="font-size:</xsl:text>
              <xsl:value-of select="$size"/>
              <xsl:text disable-output-escaping='yes'>pt;vertical-align:top"></xsl:text>
            </xsl:if>
            <xsl:if test="contains(dxl:font/@style,'subscript')">
              <xsl:variable name="size" select="number(substring-before($str,'pt')) div 2"/>
              <xsl:text disable-output-escaping='yes'>&lt;Sub style="font-size:</xsl:text>
              <xsl:value-of select="$size"/>
              <xsl:text disable-output-escaping='yes'>pt;vertical-align:sub"></xsl:text>
            </xsl:if>
          </xsl:if>
...
********************************************************************** -->
  <xsl:template match="dxl:attachmentref">
    ...
    <xsl:variable name="fileName">
      ...
      <xsl:if test="$name!=$dname">
        <xsl:value-of disable-output-escaping='yes' select="$name"/>_<xsl:value-of disable-output-escaping='yes' select="$dname"/>
      </xsl:if>
    </xsl:variable>
...
&lt;xsl:text></xsl:text>
          ...
          <xsl:otherwise>
            <xsl:if test="count(dxl:imageref/@name)>0">
              ...
              <xsl:value-of select="dxl:imageref/@name"/>
            </xsl:if>
          </xsl:otherwise>
        </xsl:choose>
        ...
        <xsl:choose>
          ...
          <xsl:when test="count(dxl:caption)!=0">
            <xsl:value-of select="number(substring-before(@height, 'px')) + 15"/>
    ...
    <xsl:element name="img">
      ...
      <xsl:attribute name="style">
        ...
        <xsl:if test="@scaledwidth">
          <xsl:text>width:</xsl:text>
          <xsl:value-of select="@scaledwidth"/>
          <xsl:text>;</xsl:text>
        </xsl:if>
        <xsl:if test="@scaledheight">
          <xsl:text>height:</xsl:text>
          <xsl:value-of select="@scaledheight"/>
          <xsl:text>;</xsl:text>
        </xsl:if>
        <!-- 境界線のスタイルを設定 -->
...
********************************************************************** -->
  <xsl:template match="dxl:table">
    ...
    <xsl:element name="div">
      ...
      <!-- tab表の場合 -->
      <xsl:variable name="tabsname">
        <xsl:value-of select="$gid"/>_tabs
      </xsl:variable>
      <xsl:if test="@rowdisplay='tabs'">
        <script>
          <xsl:text>$(function(){ $("#</xsl:text>
          <xsl:value-of select="$tabsname"/>
              ...
              <xsl:otherwise>
                <xsl:choose>
                  <xsl:when test="$colmax='1'">
                    <xsl:text>width:</xsl:text>
                    <xsl:value-of select="dxl:tablecolumn[1]/@width"/>
                    <xsl:text>;</xsl:text>
                  </xsl:when>
                  <xsl:otherwise>
	...
	-->
          ...
          <xsl:if test="count(dxl:tablecolumn)>0">
            <colgroup>
            ...
            <tr>
              ...
              <xsl:if test="$minrowheight!=''">
                <xsl:attribute name="style">
                  ...
                  <xsl:value-of select="$minrowheight"/>
                  <xsl:text>;</xsl:text>
                ...
                <td class="tablecell">
                  ...
                  <xsl:attribute name="style">
                    ...
                    <xsl:choose>
                      ...
                      <xsl:otherwise>
                        ...
                        <xsl:if test="$rowspacing!=''">
                          ...
                          <xsl:value-of select="$rowspacing"/>
                          <xsl:text>;</xsl:text>
                        ...
                        <xsl:if test="$columnspacing!=''">
                          <xsl:text>padding-left:</xsl:text>
                          <xsl:value-of select="$columnspacing"/>
                          <xsl:text>;</xsl:text>
                    ...
                    <xsl:choose>
                      ...
                      <xsl:when test="$cellborderstyle!=''">
                        <xsl:text>border-style:</xsl:text>
                        <xsl:value-of select="$cellborderstyle"/>
                        <xsl:text>;</xsl:text>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:text>border-style:solid;</xsl:text>
                  ...
                  <xsl:choose>
                    <xsl:when test="$cellcolorstyle='vgradient' or $cellcolorstyle='hgradient'">
                      ...
                      <xsl:element name="span">
                        <xsl:attribute name="style">
                          <xsl:text>display:inline-block;width:100%;height:100%;</xsl:text>
                          <xsl:call-template name="gradient">
                            <xsl:with-param name="dir" select="$dir" />
                            <xsl:with-param name="scolor" select="$cellbgcolor" />
                            <xsl:with-param name="ecolor" select="$cellaltbgcolor" />
                          </xsl:call-template>
                        </xsl:attribute>
                        <xsl:choose>
    ...
    <xsl:variable name="attriDef">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="$parNode/@def"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
      ...
      <xsl:attribute name="style">
        ...
        <xsl:choose>
          <xsl:when test="@gradientcolor!=''">
            <xsl:call-template name="gradient">
              <xsl:with-param name="dir">top</xsl:with-param>
              <xsl:with-param name="scolor" select="@color" />
              <xsl:with-param name="ecolor" select="@gradientcolor" />
            </xsl:call-template>
          </xsl:when>
          <xsl:when test="@color!=''">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:variable name="cMonth">
          <xsl:value-of select="substring($cDate, 5, 2)"/>
        </xsl:variable>
        <xsl:variable name="cDay">
          <xsl:value-of select="substring($cDate, 7, 2)"/>
        </xsl:variable>
        <xsl:variable name="cHour">
          <xsl:value-of select="substring($cDate, 10, 2)"/>
        </xsl:variable>
        <xsl:variable name="cMin">
          <xsl:value-of select="substring($cDate, 12, 2)"/>
        </xsl:variable>
        <xsl:variable name="cSec">
          <xsl:value-of select="substring($cDate, 14, 2)"/>
        </xsl:variable>
        <xsl:if test="$cHour=''">
        ...
        <xsl:choose>
          <xsl:when test="contains($reptest,'richtext:')">
            <xsl:variable name="itemname">
              <xsl:text>dxl:item[@name='</xsl:text>
              <xsl:value-of disable-output-escaping="yes" select="substring-after($reptest,'richtext:')"/>
              <xsl:text>']</xsl:text>
            </xsl:variable>
            <xsl:element name="xsl:if">
              <xsl:attribute name="test">
          ...
          <xsl:otherwise>
            <xsl:variable name="itemname">
              <xsl:text>dxl:item[@name='</xsl:text>
              <xsl:value-of disable-output-escaping="yes"  select="$reptest"/>
              <xsl:text>']</xsl:text>
            </xsl:variable>
            <xsl:element name="xsl:if">
              <xsl:attribute name="test">
                ...
                <xsl:call-template name="SubstringReplace">
                  <xsl:with-param name="text">
                    <xsl:call-template name="SubstringReplace">
                      ...
                      <xsl:with-param name="by" select="concat($amp, 'amp;')"/>
                    </xsl:call-template>
                  </xsl:with-param>
...
********************************************************************** -->
  <xsl:template match="dxl:embeddedkeywords">
    <xsl:variable name="embno">
      ...
      <xsl:number level="any" count="dxl:embeddedcontrol"/>
    </xsl:variable>
...
********************************************************************** -->
  ...
  <xsl:param name="ecolor" />
  <!--IE5-8-->
  <xsl:text>filter: progid:DXImageTransform.Microsoft.Gradient(gradientType=</xsl:text>
  <xsl:choose>
    <xsl:when test="$dir='top'">
       <xsl:text>0</xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:text>1</xsl:text>
  ...
  <xsl:text>,startColorStr=</xsl:text><xsl:value-of select="$scolor"/><xsl:text>,endColorStr=</xsl:text><xsl:value-of select="$ecolor"/><xsl:text>);</xsl:text>
  <!--IE9+-->
  <xsl:text>background-image:-ms-linear-gradient(</xsl:text><xsl:value-of select="$dir"/>
    <xsl:text>,</xsl:text><xsl:value-of select="$scolor"/>
    <xsl:text>,</xsl:text><xsl:value-of select="$ecolor"/><xsl:text>);</xsl:text>
  <!--chrome,firefox,moz,etc.-->
  <xsl:text>background-image:-webkit-linear-gradient(</xsl:text><xsl:value-of select="$dir"/>
  <xsl:text>,</xsl:text><xsl:value-of select="$scolor"/>
  <xsl:text>,</xsl:text><xsl:value-of select="$ecolor"/><xsl:text>);</xsl:text>
  <xsl:text> background-image:-moz-linear-gradient(</xsl:text><xsl:value-of select="$dir"/>
  <xsl:text>,</xsl:text><xsl:value-of select="$scolor"/>
  <xsl:text>,</xsl:text><xsl:value-of select="$ecolor"/><xsl:text>);</xsl:text>
  <xsl:text> background-image:linear-gradient(</xsl:text><xsl:value-of select="$dir"/>
  <xsl:text>,</xsl:text><xsl:value-of select="$scolor"/>
  <xsl:text>,</xsl:text><xsl:value-of select="$ecolor"/><xsl:text>);</xsl:text>
  </xsl:template>
  ...
  <xsl:template match="/">
    ...
    <xsl:text disable-output-escaping="yes"><![CDATA[<!--]]></xsl:text>
      <xsl:text disable-output-escaping="yes"><![CDATA[ <link rel="stylesheet" type="text/css" href="]]></xsl:text>
      <xsl:value-of select="$cssfile"/><xsl:text disable-output-escaping="yes"><![CDATA[" />]]></xsl:text>
      <xsl:text disable-output-escaping="yes"><![CDATA[-->]]></xsl:text>
    <xsl:apply-templates select="//dxl:form/dxl:body/dxl:richtext"></xsl:apply-templates>
  </xsl:template>
  <xsl:template match="dxl:subform">
    <xsl:apply-templates select="./dxl:body/dxl:richtext"></xsl:apply-templates>
  </xsl:template>
