...
<xsl:variable name="MaxMainParDefId">
  ...
  <xsl:template name="summary">
    ...
    <xsl:param name="Subformrefs"></xsl:param>
    <xsl:variable name="result">
      <xsl:choose>
        <xsl:when test="$index &lt;= count($Subformrefs)">
          <xsl:variable name="SubformName">
            <xsl:call-template name="GetSubForm">
              <xsl:with-param name="FullName" select="$Subformrefs[$index]/@name" />
            </xsl:call-template>
          </xsl:variable>
          <xsl:variable name="MaxId">
            <xsl:call-template name="GetMaxId">
              <xsl:with-param name="SubformName" select="$SubformName"></xsl:with-param>
            </xsl:call-template>
          </xsl:variable>
          <xsl:call-template name="summary">
            <xsl:with-param name="input" select="$input+$MaxId" />
            <xsl:with-param name="index" select="$index+1" />
            <xsl:with-param name="Subformrefs" select="$Subformrefs" />
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template name="GetOffsetId">
    <xsl:param name="key"></xsl:param>
    <xsl:call-template name="summary">
      <xsl:with-param name="input" select="$MaxMainParDefId" />
      <xsl:with-param name="index" select="1" />
      <xsl:with-param name="Subformrefs" select="//dxl:subformref[@name=$key]/preceding::dxl:subformref" />
    </xsl:call-template>
  </xsl:template>
  <!--
**********************************************************************
...
<xsl:template name="OutputOutsideAttachments">
 <xsl:element name="div">
  ...
  <xsl:element name="div">
   <xsl:attribute name="style"><xsl:text>cursor:pointer;margin-left:0px;padding-left:20px;background:url(../../domino/section_d.gif) no-repeat;zoom:1;</xsl:text></xsl:attribute>
...
<xsl:template name="OutputOutsideAttachment">
 ...
 <xsl:if test="$attachmentName!=''">
  ...
  <xsl:text>./</xsl:text><xsl:value-of select="$AttachmentDirectory"/>
  <xsl:text>/</xsl:text><xsl:value-of select="substring-after($attachmentName, ':')"/>
  <xsl:text disable-output-escaping='yes'>"></xsl:text>

   <xsl:element name="span">
    <xsl:attribute name="style">
     <xsl:text>display:inline-block;text-indent:0in;background: url(../../domino/attachicon.gif</xsl:text>
...
<xsl:template name="css-block">
  <xsl:text disable-output-escaping="yes"><![CDATA[
   body{font-size:0pt;
        ...
        font-family:]]></xsl:text><xsl:value-of select="$SansSerifFamily"/><xsl:text disable-output-escaping="yes"><![CDATA[;}
 ...
 <xsl:variable name="leftmargin">
  <xsl:choose>
   ...
   <xsl:otherwise>1in</xsl:otherwise>
	...
	<xsl:text>
		.</xsl:text><xsl:if test="$CreateMode='xsl'">Form</xsl:if><xsl:value-of select="$rtitemname"/><xsl:text>class</xsl:text>
 ...
 <xsl:for-each select="dxl:sectiontitle">
  <xsl:element name="div">
   <xsl:attribute name="class">
    <xsl:if test="$CreateMode='xsl'">Form</xsl:if><xsl:value-of select="$rtitemname"/><xsl:text>class</xsl:text><xsl:value-of select="$def"/>
   </xsl:attribute>
...
********************************************************************** -->
  <xsl:template match="dxl:subformref">
    <xsl:variable name="name"><xsl:value-of select="normalize-space(substring-before(./@name,'|'))"/></xsl:variable>
    <xsl:variable name="offsetId">
      <xsl:call-template name="GetOffsetId">
...
<xsl:template match="dxl:attachmentref">
 ...
 <xsl:choose>
  ...
  <xsl:otherwise>
   <xsl:text disable-output-escaping='yes'>&lt;a style='text-decoration:none;' target='_blank' href="./</xsl:text>
   <xsl:value-of select="$unid"/><xsl:text>/</xsl:text><xsl:value-of select="$fileName"/>
   <xsl:text disable-output-escaping='yes'>"></xsl:text>
  </xsl:otherwise>
 </xsl:choose>
