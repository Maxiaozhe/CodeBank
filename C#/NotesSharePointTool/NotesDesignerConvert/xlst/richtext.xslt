...
********************************************************************** -->
  ...
  <!-- Subformの採番ID-->
  <xsl:param name="offsetId"></xsl:param>
  
  <!--イメージパス-->
  <xsl:param name="imgurl">../../domino/</xsl:param>
  <!--
**********************************************************************
変数定義
********************************************************************** -->
  ...
  <!-- FormのParDefの最大ID-->
  <xsl:variable name="MaxMainParDefId">
    <xsl:for-each select="//dxl:form/descendant::dxl:pardef">
  ...
  <xsl:template name="GetMaxId">
    <xsl:param name="SubformName"></xsl:param>
    <xsl:choose>
      <xsl:when test="count(//dxl:subform[@name=$SubformName]/descendant::dxl:pardef)&gt;0">
        <xsl:value-of select="0"/>
      </xsl:when>
      <xsl:otherwise></xsl:otherwise>
    </xsl:choose>
    <xsl:for-each select="//dxl:subform[@name=$SubformName]/descendant::dxl:pardef">
      <xsl:sort order="descending" select="number(./@id)"/>
      <xsl:if test="position()=1">
        <xsl:value-of select="number(./@id)"/>
      </xsl:if>
  ...
  <xsl:template match="dxl:database">
    <xsl:choose>
      <xsl:when test="$formName!=''">
        <xsl:if test="count(./dxl:form[@name=$formName])>0">
          <xsl:apply-templates select="./dxl:form[@name=$formName]" />
        </xsl:if>  
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="count(./dxl:form)>0">
          <xsl:apply-templates select="./dxl:form[position()=1]" />
        </xsl:if>
      </xsl:otherwise>  
    </xsl:choose>
  </xsl:template>
        ...
        <xsl:element name="div">
          ...
          <xsl:attribute name="style">
            ...
            <xsl:if test="@borderstyle='twoline' 
              ...
              <xsl:text>background:url(</xsl:text><xsl:value-of select="$imgurl"/>
              <xsl:choose>
                <xsl:when test="$onread='collapse'">
                  <xsl:text>section_r.gif) no-repeat;zoom:1;</xsl:text>
                </xsl:when>
            ...
            <xsl:attribute name="style">
              ...
              <xsl:choose>
                ...
                <xsl:when test="@borderstyle='gradient'">
                  ...
                  <xsl:value-of select="@color"/>
                  <xsl:text>,endColorStr=#FFFFFF);</xsl:text>
                  <xsl:text>background-image:-webkit-linear-gradient(left,</xsl:text>
                  <xsl:value-of select="@color"/>
...
form
********************************************************************** -->
  <xsl:template match="dxl:form" name="form">
    <html>
      <title>
  ...
  <xsl:template match="dxl:body" name="body">
    <body>
      <xsl:apply-imports/>
    </body>
  </xsl:template>
...
********************************************************************** -->
  <xsl:template match="dxl:subformref">
    ...
    <xsl:apply-templates select="//dxl:subform[@name=$name]/dxl:body" ></xsl:apply-templates>
  </xsl:template>
...
********************************************************************** -->
  <xsl:template match="dxl:run">
    <xsl:choose>
      <xsl:when test="count(child::dxl:field)!=0">
        <xsl:apply-imports/>
      </xsl:when>
  ...
  <xsl:template name="outputImage">
    ...
    -->
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:attribute name="src">
            <xsl:text>./</xsl:text><xsl:value-of select="$unid"/><xsl:text>/</xsl:text>
            <xsl:value-of select="$image_fix"/><xsl:value-of select="$imgnum"/>.<xsl:apply-templates select="dxl:gif|dxl:jpeg|dxl:png|dxl:tif"/>
          </xsl:attribute>
        </xsl:otherwise>
      </xsl:choose>
                    ...
                    <xsl:when test="$cellcolorstyle='vgradient' or $cellcolorstyle='hgradient'">
                      <xsl:element name="span">
                        <xsl:attribute name="style">
                          ...
                          <xsl:value-of select="$cellaltbgcolor"/>
                          <xsl:text>);</xsl:text>
...
********************************************************************** -->
  ...
  <xsl:template match="dxl:rawitemdata">
    <xsl:comment>
      <xsl:apply-imports/>
    </xsl:comment>
  </xsl:template>
  <!--フォーム作成情報-->
  <xsl:template match="dxl:created">
    <xsl:comment>
      created：<xsl:apply-imports/>
    </xsl:comment>
  </xsl:template>
  <xsl:template match="dxl:modified">
    <xsl:comment>
      modified：<xsl:apply-imports/>
    </xsl:comment>
  </xsl:template>
  <xsl:template match="dxl:revised">
    <xsl:comment>
      revised：<xsl:apply-imports/>
    </xsl:comment>
  </xsl:template>
  <xsl:template match="dxl:lastaccessed">
    <xsl:comment>
      revised：<xsl:apply-imports/>
    </xsl:comment>
  </xsl:template>
  <xsl:template match="dxl:addedtofile">
    <xsl:comment>
      revised：<xsl:apply-imports/>
    </xsl:comment>
  </xsl:template>
  <xsl:template match="dxl:updatedby">
    <xsl:comment>
      updatedby：<xsl:apply-imports/>
    </xsl:comment>
  </xsl:template>
</xsl:stylesheet>