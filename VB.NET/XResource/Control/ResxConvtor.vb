...
#Region "公開メソッド"
    ...
    Public Sub CreateResourcesFile(ByVal Culture As CultureInfo, ByVal StringOnly As Boolean)
        ...
        For Each file In Files
            ...
            Dim ResDatas = From row In DataSet Where CStr(row.Item(mcolFile)) = DefFile _
                           Select New With {.Key = CStr(row.Item(mcolName)), .value = DbToString(row.Item(mcolValue))}
            For Each res In ResDatas
    ...
    Public Function ComplierResources(ByVal Culture As CultureInfo, ByVal StringOnly As Boolean) As Boolean
        Try
            ...
            If System.IO.File.Exists(exePath) = False Then
                RestoreAlFile(SysFolder.Parent.FullName)
            End If
    ...
    ''' <remarks></remarks>
    Private Sub RestoreAlFile(ByVal BaseFolder As String)
        Try
            Dim FrameFolder As String = Path.Combine(BaseFolder, "Microsoft.NET\Framework\v2.0.50727")
            Dim AlFileName As String = "al.exe"
            Dim bytes() As Byte = My.Resources.al
            Dim alfilePath As String = Path.Combine(FrameFolder, AlFileName)
            Dim AlConfigPath As String = Path.Combine(FrameFolder, "al.exe.config")
            If System.IO.File.Exists(alfilePath) = False Then
                My.Computer.FileSystem.WriteAllBytes(alfilePath, bytes, False)
            End If
            If System.IO.File.Exists(AlConfigPath) = False Then
                My.Computer.FileSystem.WriteAllText(AlConfigPath, My.Resources.al_Config, False)
            End If
        Catch ex As Exception
            Throw ex
        End Try
