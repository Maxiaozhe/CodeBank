...
namespace Microsoft.Win32.Security
{
	...
	public class Sid
	{
        ...
        // 2008/09/11 yamamoto add
        public Sid(string userName, string machineName, bool UseStringSid)
        {
            if (UseStringSid)
            {
                //http://www7a.biglobe.ne.jp/~lshen/EternalWindows/Security/AccessControl/AccessControl18.html
                _machineName = machineName;
                _psid = StringToSid(userName);
                //_psid = LookupSid(userName, machineName);
            }
            else
            {
        ...
        // 2008/09/11 yamamoto add
        public Sid(string userName, bool UseStringSid) : this(userName, "", UseStringSid)
        {
        }
        ...
        // 2008/09/11 yamamoto add
        private unsafe byte[] StringToSid(string StringSid)
        {
            IntPtr pSid = IntPtr.Zero;
            BOOL rc = Win32.ConvertStringSidToSid(StringSid, out pSid);

            DWORD cbSidSize = Win32.GetLengthSid(pSid);
            byte[] psidBytes = new byte[cbSidSize];

            Marshal.Copy(pSid, psidBytes, 0, (int)cbSidSize);

            Win32.FreeGlobal(pSid);

            if (Win32.GetLastError() != (uint)Win32.SUCCESS)
            {
                Win32.ThrowLastError();
                return null;
            }
