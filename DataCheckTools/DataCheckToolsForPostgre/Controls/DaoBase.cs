...
using System.Text;
namespace Rex.Tools.Test.DataCheck.Controls.SqlServer
{
    /// <summary>
    /// SQL アクセサーを実装する
    /// </summary>
    public class DaoBase : IDisposable
    {
        ...
        internal SqlConnection Connection
        {
            get
            {
                ...
                {
                    this._sqlConnect = new SqlConnection(this._ConnString);
        ...
        /// <returns></returns>
        internal DaoBase(string connectString, int commandTimeOut)
        {
        ...
        /// <returns></returns>
        public SqlDataReader SelectSql(string rstrSQL)
        {
            try
            {
                if (!this._IsConnected)
                ...
                using (SqlCommand cmd = this.CreateCommand())
                {
        ...
        public DataTable SelectSql(string sql, string tableName)
        {
            ...
            try
            {
                ...
                {
                    cmd.CommandText = sql;
                    adapter = new SqlDataAdapter(sql, this._ConnString);
        ...
        public bool SelectSql(string rstrSQL, string rstrTableName, DataSet dataset, bool IsFillSchema)
        {
            ...
            try
            {
                ...
                using (SqlCommand cmd = this.CreateCommand())
                {
                    cmd.CommandText = rstrSQL;
                    cmd.CommandTimeout = this._CommandTimeout;
                    adapter = new SqlDataAdapter(rstrSQL, this._ConnString);
                    adapter.SelectCommand = cmd;
                    if (IsFillSchema)
                    {
        ...
        public virtual DataSet ExecuteResultSet(SqlCommand cmd)
        {
            DataSet dts;
            SqlDataAdapter adapter = null;
            try
        ...
        public virtual DataSet ExecuteResultSet(SqlCommand cmd, bool IsFillSchema)
        {
            DataSet dts = null;
            SqlDataAdapter adapter = null;
            try
        ...
        public virtual DataTable ExecuteResultSet(SqlCommand cmd,string TableName, bool IsFillSchema)
        {
            return ExecuteResultSet(cmd, new DataSet(), TableName, IsFillSchema);
        }

        public virtual DataTable ExecuteResultSet(SqlCommand cmd, DataSet rDataSet, string TableName, bool IsFillSchema)
        {
            SqlDataAdapter adapter = null;
            try
            {
        ...
        #region UpdateTable
        public virtual int UpdateTable(DataTable rdatTable, SqlCommand SelectCommand)
        {
            SqlDataAdapter adapter = null;
        ...
        public virtual int UpdateTable(DataTable rdatTable, SqlCommand SelectCommand, SqlCommand UpdateCommand, SqlCommand InsertCommand, SqlCommand DeleteCommand)
        {
            SqlDataAdapter adapter = null;
            try
            {
