...
namespace Rex.Tools.Test.DataCheck.Forms
{
    ...
    public partial class MenuForm : BaseForm
    {
        ...
        internal enum CommandKind
        {
            ...
            /// 伝匠データシート作成
            /// </summary>
            DenshowDataSheetCreate,
            /// <summary>
            /// 伝匠INSERT SQL作成
            /// </summary>
            DenshowInsertSqlCreate,
        }
        ...
        /// <param name="sheets"></param>
        /// <param name="isTarget"></param>
        private void DoCreateDenshowDataSheet(string[] sheets)
        {

            NpgsqlDataSheetCreator creator = new NpgsqlDataSheetCreator(reportHandler);
            creator.CreateDBInitData(sheets);
        }
        ...
        /// <param name="designFileName"></param>
        /// <param name="layoutType"></param>
        private void DoDenshowInsertSqlScript(string dataSheetName)
        {
            NpgTestDataInitializer initializer = new NpgTestDataInitializer(reportHandler);
            initializer.DoInitlize(dataSheetName, "");
        }
        ...
        private void btnDenshow_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SheetSelectForm form = new SheetSelectForm())
                {
                    form.Title = "伝匠　テーブル一覧を選択する";
                    form.ExcelFileName = Config.GetDenshowSettingFile();
                    if (form.ShowDialog(this) == System.Windows.Forms.DialogResult.Cancel)
                    {
                        return;
                    }
                ...
                Run(new CommandArg(CommandKind.DenshowDataSheetCreate, selectedSheets));
            }
            catch (Exception ex)
            {
                Logging.Exception("", ex);
        ...
        private void dtnDenshowInsertDataCreate_Click(object sender, EventArgs e)
        {
            try
            {
                this.openExcelDialog.Title = "伝匠 データ入力シートを選択してください";
                if (this.openExcelDialog.ShowDialog(this) != DialogResult.OK)
                {
                    return;
                }
                if (System.IO.File.Exists(this.openExcelDialog.FileName))
                {
                    Run(new CommandArg(CommandKind.DenshowInsertSqlCreate, this.openExcelDialog.FileName));
                }
            }
            catch (Exception ex)
            {
